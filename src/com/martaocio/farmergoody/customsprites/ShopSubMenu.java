...
public class ShopSubMenu extends Sprite {
	...
	public ShopSubMenu(float pX, float pY, ITextureRegion texture, VertexBufferObjectManager vbom, MenuScene parentScene,
			MainMenu parentMenu) {
	...
	private Sprite createShopMenuItem(final Vehicle vehicle, boolean alreadyHad, boolean canBuy, final int indexItem) {
		// AnimatedSprite itemBackground = new AnimatedSprite(0, 0,
		// Vehicle.getVehicleShopItem(vehicle), vbom);
		...
		if (canBuy && !alreadyHad) {
			...
			// buyBtn.setPosition(shopItem.getWidth()-30,
			// shopItem.getHeight()-30);
	...
	public void buyVehicle(Sprite touchedSprite) {
		...
		if (!UserState.getInstance().getAvailableVehicles().contains(vehicleSelected)) {
